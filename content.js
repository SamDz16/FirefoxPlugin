...
if (location.href === 'https://dbpedia.org/sparql') {
	...
	isFailingDiv.classList.add('form-check', 'form-check-inline');
	isFailingDiv.style.margin = '10px 0';

	// Creat the label element
	const isFailingLabel = document.createElement('label');
	isFailingLabel.classList.add('form-check-label');
	isFailingLabel.setAttribute('for', 'algo');
	isFailingLabel.append(
		'Do you want to use Base, BFS and VAR algorithms to compute XSSs and MFISs ?'
	);
	// isFailingLabel.style.marginLeft = '10px';

	// Create the checkbox element
	const isFailingInput = document.createElement('input');
	isFailingInput.classList.add('form-check-input');
	...
	KDiv.classList.add('form-group', 'row', 'd-none');
	KDiv.style.margin = '10px 0';
	KDiv.id = 'K';

	// Label of threshold K
	const KLabel = document.createElement('label');
	KLabel.setAttribute('for', 'k');
	KLabel.append('Choose the threshold K (number of expected results)');
	// KLabel.style.margin = '10px 10px 10px 0';

	const KInput = document.createElement('input');
	KInput.classList.add('form-control');
	...
	const isFailingAlgorithm = async (e) => {
		...
		try {
			...
			addTitleToDom(document.querySelector('#options'), {
			...
			document
				...
				.append(
					insertToDOM(
						{ id: 'isfailing', value: isfailing, text: 'IsFailing returns ?' },
						0
					)
				);
			...
			document
				...
				.append(
					insertToDOM(
						{ id: 'xss', value: resGlobal[0], text: 'List of XSS' },
						15
					)
				);
			document
				...
				.append(
					insertToDOM(
						{ id: 'mfis', value: resGlobal[1], text: 'List of MFIS' },
						15
					)
				);
			addTitleToDom(
			...
			document.querySelector('#options').append(
				insertToDOM(
					{
						id: 'nb',
						value: resGlobal[2],
						text: 'Number of executed queries',
					},
					0
				)
			);
				...
				insertToDOM(
					{
						id: 'etMakeLattice',
						value: resGlobal[3],
						text: 'Execution Time for Make Lattice algorithm',
					},
					0
				)
			);
				...
				insertToDOM(
					{
						id: 'etAllQueries',
						value: resGlobal[4],
						text: `Execution Time for All executed querries ${resGlobal[2]}`,
					},
					0
				)
			);
	...
	document
		...
		.insertAdjacentElement('beforebegin', document.createElement('hr'));

	const h1Full = document.createElement('h1');
	h1Full.textContent = 'Full ALGORITHM';
	document
		.querySelector('#footer')
		.insertAdjacentElement('beforebegin', h1Full);
	...
	document
		...
		.insertAdjacentElement(
			'beforebegin',
			insertToDOM(
				{ id: 'cardinalities', value: '', text: 'Cardinalities' },
				7,
				false
			)
		);
	...
	document
		.querySelector('#footer')
		.insertAdjacentElement('beforebegin', FullAlgoBtn);
	...
	document.querySelector('#full').addEventListener('click', async () => {
		...
		document
			...
			.insertAdjacentElement(
				'beforebegin',
				insertToDOM({ id: 'xss', value: resGlobal[0], text: 'List of XSS' }, 15)
			);
		document
			.querySelector('#footer')
			.insertAdjacentElement(
		...
		document
			.querySelector('#footer')
			.insertAdjacentElement('beforebegin', h3Full);
		...
		document
			...
			.insertAdjacentElement(
				'beforebegin',
				insertToDOM(
					{ id: 'nb', value: resGlobal[2], text: 'Number of executed queries' },
					0
				)
			);
		document.querySelector('#footer').insertAdjacentElement(
			'beforebegin',
			insertToDOM(
				{
					id: 'etMakeLattice',
...
if (location.href === 'http://localhost:3030/dataset.html') {
	...
	btnQuery.addEventListener('click', async () => {
		...
		container.append(
			insertToDOM({ id: 'mfis', value: resGlobal[1], text: 'List of MFIS' }, 15)
		);
		...
		container.append(
			insertToDOM({
				id: 'etAllQueries',
				value: resGlobal[4],
				text: 'Execution Time for All subqueries of the initial query',
			})
		);
